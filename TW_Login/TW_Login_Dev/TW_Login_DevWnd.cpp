...
// CTW_LoginWnd
	...
	IMPLEMENT_DYNAMIC(CTW_Login_DevWnd, CWnd)

	BEGIN_DISPATCH_MAP(CTW_Login_DevWnd, CWnd)
		DISP_FUNCTION_ID(CTW_Login_DevWnd, "OnQuit", DISPID_ONQUIT, OnQuit, VT_EMPTY, VTS_NONE)
		DISP_FUNCTION_ID(CTW_Login_DevWnd, "OnDocumentComplete", DISPID_DOCUMENTCOMPLETE, OnDocumentComplete, VT_EMPTY, VTS_DISPATCH VTS_PVARIANT)
	END_DISPATCH_MAP()

CTW_Login_DevWnd::CTW_Login_DevWnd()
{
	EnableAutomation();
...
BEGIN_MESSAGE_MAP(CTW_Login_DevWnd, CWnd)
END_MESSAGE_MAP()

// CTW_LoginWnd message handlers

// 로그인 함수, 최종 로그인은 버튼을 클릭한 것처럼 만들어서 자동로그인 되도록 한다.
BOOL CTW_Login_DevWnd::AutoLoginByButton(CString strUrl, CString strID, CString strPwd, CString strFormName, CString strIDObjectID, CString strPwdObjectID, CString strClickObjectID)
{
...
BOOL CTW_Login_DevWnd::AutoLoginByScript(CString strUrl, CString strID, CString strPwd, CString strFormName, CString strIDObjectID, CString strPwdObjectID, CString strClickObjectID)
{
	this->cstr_Url = strUrl;
	this->cstr_ID = strID;
	this->cstr_Pwd = strPwd;
...
// 새로운 웹 브라우저를 실행 시킨다.
BOOL CTW_Login_DevWnd::CreateNewBrowser()
{
...
// 이벤트 싱크
BOOL CTW_Login_DevWnd::AdviseBrowser()
{
	if (m_pWebBrowser2)
	{
...
// 이벤트 싱크 해제
BOOL CTW_Login_DevWnd::UnAdviseBrowser()
{
...
// 웹 탐색을 한다.
void CTW_Login_DevWnd::NavigateWeb()
{
...
// 웹 페이지 로딩이 완료되면
void CTW_Login_DevWnd::OnDocumentComplete(IDispatch *pDisp,VARIANT *URL)
{
...
BOOL CTW_Login_DevWnd::PutText(CString strFormName, CString strObjectID, CString strPutText)
{
...
BOOL CTW_Login_DevWnd::LoginByButton()
{
...
// 스크립트를 이용하여 자동실행 시킨다.
BOOL CTW_Login_DevWnd::LoginByScript()
{
	CComPtr<IHTMLElement> pElement = NULL;
	CComPtr<IDispatch> pDisp = NULL;
	DISPID dispid = NULL;
